...
<?php
	...
	if (isset($v_global)){
		if ($v_global==Ajout){
		$code_nom_client=recupere_code_popup_consigne($g_nom_du_client);		
			if ($code_nom_client!="") {
				if ($g_naturobjet!=""){
					if ($g_nombrobjet!=""){
						if ($g_datedepot!=""){
							...
							ajout_table_consigne($code_nom_client,$g_naturobjet,$g_nombrobjet,$g_datedepot_formatee_visiteur,$g_heuredepot,$g_dateretrait_formatee_visiteur,$g_heureretrait,$g_observation);			
				
							print "<script language='javascript'> alert('Enregistrement effectue avec succes');</script>";
							$code_nom_client="$code_nom_client";
							$g_naturobjet="$g_naturobjet";
							$g_nombrobjet="$g_nombrobjet";
							$g_datedepot="$g_datedepot";							
							header('location: consigne.php');
						}
						else { 

							print"<script language='javascript'> alert('Veuillez selectionner la date de depot');</script>"; 
						}
					}
						else { 
							print"<script language='javascript'> alert('Veuillez saisir la quantite');</script>"; 
						}
				}
						else { 
							print"<script language='javascript'> alert('Veuillez saisir la nature');</script>"; 
						}
			}
			else { 
					print"<script language='javascript'> alert('Veuillez choisir le nom du client');</script>"; 
				}
		...
		elseif ($v_global==Modif) {
				...
				$code_nom_client=recupere_code_popup_consigne($g_nom_du_client);
					if ($g_naturobjet!="") {
						$g_datedepot_formatee_visiteur=dateFormBase($g_datedepot);
						$g_dateretrait_formatee_visiteur=dateFormBase($g_dateretrait);
						modif_table_consigne($code_nom_client,$g_naturobjet,$g_nombrobjet,$g_datedepot_formatee_visiteur,$g_heuredepot,$g_dateretrait_formatee_visiteur,$g_heureretrait,$g_observation,$g_code);
				
				print "<script language='javascript'> alert('Modification effectuee avec succes'); </script>";
			}
		}
		elseif ($v_global==Suppression){
			suppression_table_consigne($g_code);
	...
	if ($i==0) {
		if (isset($g_code)){			
			$code=$g_code;
			$g_nom_du_client=recupere_nom_client_table_consigne($g_code);
			$g_naturobjet=recupere_nature_objet($g_code);
			$g_nombrobjet=recupere_nombre_objet($g_code);
			$g_datedepot=recupere_date_depot($g_code);
			$g_heuredepot=recupere_heure_depot($g_code);
			$g_dateretrait=recupere_date_retrait($g_code);
			$g_heureretrait=recupere_heure_retrait($g_code);
			$g_observation=recupere_observation($g_code);
  ...
  ?>
                          <div class="art-post-inner art-article">
                                          <h2 class="art-postheader" style="text-align:center;">GESTION DES CONSIGNES<a href="#" rel="bookmark" title="Permanent Link to this Post"></a> <a class="visited" href="#" rel="bookmark" title="Visited Hyperlink"></a> <a class="hovered" href="#" rel="bookmark" title="Hovered Hyperlink"></a></h2>
                                          		  <?php 
									   require_once('../procedure_php/procedure_globale.php');
										$link=Connexion();
										...
										print " <th class='entete_tableau' >";
										print " <strong> Date de d&eacute;pot </strong>";
	           							print " </th>";
										...
										$requete="select ID_CONSIGNE, CONCAT(NOM_CLIENT,' ',PRENOM_CLIENT) as NOM_CLIENT, NATURE_OBJET, NOMBRE_OBJET, DATE_DEPOT, DATE_RETRAIT, OBSERVATION_OBJET from consigne,personne_physique where consigne.ID_PERSONNE=personne_physique.ID_PERSONNE order by ID_CONSIGNE DESC LIMIT $limit_start, $pagination";
										...
										{  extract($lecture);
											...
											// print " <td>".$ID_CONSIGNE."</td>";
											 print " <td> <a href='consigne.php?modif_code=$ID_CONSIGNE'> ".stripslashes($NOM_CLIENT)." </a></td>";
											 print " <td>".stripslashes($NATURE_OBJET)."</td>";
											 print " <td>".stripslashes($NOMBRE_OBJET)."</td>";
											 print " <td>".stripslashes($DATE_DEPOT)."</td>";
											 print " <td>".stripslashes($DATE_RETRAIT)."</td>";
											 print " <td>".stripslashes($OBSERVATION_OBJET)."</td>";
										...
										print "Nombre de consignes : ".$i." sur"." $result";
										
										// Début de la pagination
										// Nb d'enregistrement total
										$nb_total = mysql_query('SELECT COUNT(*) AS nb_total FROM consigne');
                          ...
                          <div class="art-post-inner art-article"><strong  style="text-align:center;" ></strong>
										  ...
										  <tr style="border:hidden">
												<form method="post" name="consigne" action="consigne.php?modif_code=<?php print $g_code; ?>" >
													<p align="center">
														 <input type="text" name="txt_code" value="<?php print $g_code; ?>" class="le_code" />
														 <td style="border:hidden"><label> Nom du client : </label></td>
														 <td style="border:hidden"><input readonly type="text" id="nom_du_client" name="txt_numidentite" value="<?php print $g_nom_du_client; ?>"  size="33" /><input type="button" value="..." onClick="openPopup()" style="width:46px; height:21px" /> <!-- <input type="submit" style="width:46px; height:21px" name="btn"  value="..." onClick="window.open('../formulaire/liste_client_visite.php','Liste des clients','toolbar=no,menubar=no,location=no,risizable=no,scrollbars=no,status=no,height=356,width=350')" /> --></td>
															<!-- <td><select name="txt_numidentite">												
																	<option>Choisir le client</option> 															 
																		<?php print $g_nom_du_client; ?>
																		<?php ChargeCombo ("select NOM_CLIENT as info from personne_physique",$g_nom_du_client); ?>
																</select>
															</td> -->
														<!-- <td><label> Nom du client : </label></td>
														<td><input type="text" name="txt_numidentite" value="<?php print $g_nom_du_client; ?>"  size="33" /></td> -->
												    </p>
											</tr>
										  <tr style="border:hidden">
													<p align="center">
											...
											<tr style="border:hidden">
													<p align="center">
														...
														<?php include_once("../calendrierphp/calendar.php"); calendarpopupDim('id1','document.consigne.txt_datedepot',"fr","1","0"); ?>
											...
											<tr style="border:hidden">
													<p align="center">
														...
														<?php include_once("../calendrierphp/calendar.php"); calendarpopupDim('id1','document.consigne.txt_dateretrait',"fr","1","0"); ?>
										...
										<input type="submit" name="btn"   style="width:80px; height:30px" value="Supprimer" onclick="return confirm('Etes-vous sur de vouloir supprimer cette consigne?');"/>
										<input type="reset" name="btn_Annuler" style="width:80px; height:30px" value=" Annuler "/>
															
										<input type="submit" name="btn_Imprimer" style="width:80px; height:30px" onClick="window.open('../etat/liste_consigne.php','Liste des consignes','toolbar=no,menubar=no,location=no,risizable=no,scrollbars=no,status=no,height=800,width=800')" value="Imprimer" />
									</form> 
